...
func (mw *MainWindow) getRunes(r *[]models.R) error {
	rows := mw.db.Find(r)
	if rows.Error != nil {
...
func (mw *MainWindow) getRuneWords(rw *[]models.RW) error {
	rows := mw.db.Find(rw)
	if rows.Error != nil {
		fmt.Println("No rows:", rows.Error)
		return rows.Error
...
func (mw *MainWindow) showAllRW(c *fyne.Container, data *[]models.RW) {
	var desc, ls string
	c.RemoveAll()
